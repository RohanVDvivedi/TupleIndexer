...
{
	const bplus_tree_page_header* ph = get_page_header((void*)page, page_size) + get_size_of_page_type_header();
	return ph->level;
}

void set_level_of_bplus_tree_page(void* page, uint32_t page_size, uint32_t level)
{
	bplus_tree_page_header* ph = get_page_header(page, page_size) + get_size_of_page_type_header();
	ph->level = level;
}
