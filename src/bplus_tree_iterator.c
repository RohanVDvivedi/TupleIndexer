...
int next_bplus_tree_iterator(bplus_tree_iterator* bpi_p, const void* transaction_id, int* abort_error)
{
	...
	while(1)
	{
		...
		if(next_page_id == bpi_p->bpttd_p->NULL_PAGE_ID) // we reached the end of leaf page list
			return 0;
		else
		{
...
{
	if(bpi_p->curr_tuple_index >= get_tuple_count_on_persistent_page(&(bpi_p->curr_page), bpi_p->bpttd_p->page_size, &(bpi_p->bpttd_p->record_def->size_def)))
		return NULL;
...
int prev_bplus_tree_iterator(bplus_tree_iterator* bpi_p, const void* transaction_id, int* abort_error)
{
	...
	while(1)
	{
		...
		if(prev_page_id == bpi_p->bpttd_p->NULL_PAGE_ID) // we reached the end of leaf page list
			return 0;
		else
		{
