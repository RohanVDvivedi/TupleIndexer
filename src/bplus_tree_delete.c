...
int delete_from_bplus_tree(uint64_t root_page_id, const void* key, const bplus_tree_tuple_defs* bpttd_p, const data_access_methods* dam_p, const page_modification_methods* pmm_p, const void* transaction_id, int* abort_error)
{
	...
	walk_down_locking_parent_pages_for_merge_using_key(root_page_id, locked_pages_stack_p, key, bpttd_p, dam_p, transaction_id, abort_error);
	if(*abort_error)
	{
	...
	int result = merge_and_unlock_pages_up(root_page_id, locked_pages_stack_p, bpttd_p, dam_p, pmm_p, transaction_id, abort_error);
	...
	// return merge result, which has to be 1, in absence of an abort_error
	return result;
