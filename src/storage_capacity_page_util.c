...
#include<page_layout_read_only.h>
#include<tuple.h>
...
{
	uint32_t allotted_space = get_space_allotted_to_all_tuples_on_page(page, page_size, &(index_def->size_def));
	uint32_t used_space = get_space_occupied_by_all_tuples_on_page(page, page_size, &(index_def->size_def));

	// if the page is already lesser than half full, then it may require merging or redistribution
	if(used_space <= (allotted_space / 2))
		return 1;

	uint32_t tuple_count = get_tuple_count_on_page(page, page_size, &(index_def->size_def));
	...
	if(child_index < tuple_count)
	{
		...
		uint32_t new_used_space = used_space - get_space_occupied_by_tuples_on_page(page, page_size, &(index_def->size_def), child_index, child_index);
	...
	{
		// find new used space after deleting the tuple at the child_index (and after full compaction and tomb stone removal)
		uint32_t new_used_space = used_space - get_space_occupied_by_tuples_on_page(page, page_size, &(index_def->size_def), child_index + 1, child_index + 1);
