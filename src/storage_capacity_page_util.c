...
{
	return ((float)get_space_occupied_by_all_tuples(page, page_size, def)) / get_space_allotted_to_all_tuples(page, page_size, def);
}

float get_storage_efficiency_for_page(const void* page, uint32_t page_size, const tuple_def* def)
{
	uint32_t space_occupied_by_all_tuples = get_space_occupied_by_all_tuples(page, page_size, def);
	uint32_t space_used_by_all_tuples = get_space_allotted_to_all_tuples(page, page_size, def) - get_free_space_in_page(page, page_size, def);

	return ((float)space_occupied_by_all_tuples) / space_used_by_all_tuples;
}