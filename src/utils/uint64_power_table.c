...
int initialize_power_table_uint64(power_table_uint64* ptu_p, uint64_t N)
{
	...
	ptu_p->power_table[0] = N;

	for(ptu_p->overflows_at = 1; ptu_p->overflows_at < sizeof(ptu_p->power_table)/sizeof(ptu_p->power_table[0]); ptu_p->overflows_at++)
	{
		if(will_unsigned_mul_overflow(uint64_t, ptu_p->power_table[ptu_p->overflows_at-1], ptu_p->power_table[ptu_p->overflows_at-1]))
			break;

		// power_table[i] = power_table[0] ^ (2 ^ i) = (power_table[0] ^ (2 ^ (i-1))) ^ 2 = power_table[i-1] ^ 2
		ptu_p->power_table[ptu_p->overflows_at] = ptu_p->power_table[ptu_p->overflows_at-1] * ptu_p->power_table[ptu_p->overflows_at-1];
	}
...
int get_power_using_power_table_uint64(power_table_uint64* ptu_p, uint64_t exponent, uint64_t* result)
{
	...
	for(uint8_t bit_index = 0; bit_index < (sizeof(uint64_t) * CHAR_BIT) && ((exponent >> bit_index) != UINT64_C(0)); bit_index++)
	{
		...
		if(bit_index >= ptu_p->overflows_at)
			return 0;
		...
		if(bit_value)
		{
			...
			if(will_unsigned_mul_overflow(uint64_t, ptu_p->power_table[bit_index], (*result)))
				return 0;

			(*result) *= ptu_p->power_table[bit_index];
		}
	}
