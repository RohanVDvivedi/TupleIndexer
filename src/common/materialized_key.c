...
materialized_key materialize_key_from_tuple(const void* tuple, const tuple_def* tpl_d, const positional_accessor* key_columns_to_materialize, uint32_t key_element_count)
{
	...
	{
		mat_key.key_dtis[i] = get_type_info_for_element_from_tuple_def(tpl_d, key_columns_to_materialize[i]);
		get_value_from_element_from_tuple(&(mat_key.keys[i]), tpl_d, key_columns_to_materialize[i], tuple);
	}
