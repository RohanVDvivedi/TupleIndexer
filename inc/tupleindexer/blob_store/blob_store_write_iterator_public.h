...
int is_empty_blob_in_blob_store(const blob_store_read_iterator* bsri_p);

// returns head_page_id
uint64_t get_head_position_in_blob(blob_store_read_iterator* bsri_p, uint32_t* head_tuple_index);

// tail_byte_index will be the size of the binary in the tail
uint64_t get_tail_position_in_blob(blob_store_read_iterator* bsri_p, uint32_t* tail_tuple_index, uint32_t* tail_byte_index);
